...
def auto_summarize(file, model_name, summary_state):
    ...
    for chunk, new_summary_state in summary_generator:
        summary = chunk
        # Disable the Ask button during summarization
        yield (
        ...
        if len(new_summary_state.split()) > 150 and not summary_complete:
            ...
            # Preload the context by sending a dummy question
            try:
                for _ in query_ollama_model("What is the main idea of the paper?", model_name, context=new_summary_state, stream=False):
                    pass  # Discard the response; we only care about preloading the context
            except Exception as e:
                print(f"Error preloading context: {str(e)}")

    # Finalize suggestions after the summary is complete
    if not suggestions:
        generate_suggestions_after_summary()

    # Enable the Ask button and provide suggestions
    yield (
        summary,  # Chatbot output
...
def handle_suggestion_selection(selected_question, model_name, chat_history, summary_state):
    ...
    # Regenerate follow-up suggestions after the response
    new_suggestions = generate_follow_up_suggestions(summary_state, model_name)
    # Finalize by enabling the inputs and updating suggestions
    yield (
        chat_history,  # Chatbot output
        gr.update(value=""),  # Clear the input field
        gr.update(choices=new_suggestions, interactive=True),  # Re-enable follow-up suggestions dropdown with new suggestions
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Research Paper Assistant"):
        ...
        pdf_upload.change(
            ...
            outputs=[chatbot, follow_up_suggestions, user_input, submit_button, summary_state]
        )
        ...
        follow_up_suggestions.change(
            ...
            outputs=[chatbot, user_input, follow_up_suggestions, user_input, submit_button]
        )
